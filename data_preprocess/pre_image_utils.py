...
def change_filename_with_class(src_dir, dst_dir, defect_name, size=(512,512)):
    """Resize and rename images by class with sequential numbering"""
    classes = os.listdir(src_dir)
    
    idx=0
    for class_name in classes:
        dest_image_dir = os.path.join(dst_dir, 'images', class_name)
...
def resize_images(src_dir, dst_dir, size=(512, 512)):
        ...
        for file_name in fileList:
            ...
            if file_name.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                ...
                if img is not None:
                    ...
                    output_path = os.path.join(dst_img_dir, f'{filename}.png')
...
if __name__ == "__main__":
    ...
    if TASK == "resize":
        print(f"Resizing images from: {src_dir}")
        resize_images(src_dir=src_dir, 
                      dst_dir=dst_dir,
                      size=(512, 512))
